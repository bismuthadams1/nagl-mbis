...
def main(output: str):
    ...
    data_set_iodines = client.get_dataset(dataset_type='singlepoint',dataset_name='MLPepper RECAP Optimized Fragments v1.0 Add Iodines')
    ...
    with pyarrow.parquet.ParquetWriter(where=output, schema=schema) as writer:
        ...
        print('writing iodines now')
        with ProcessPoolExecutor(max_workers=16) as pool:
            # process in 1000 batch chunks
            for batch in batched(entries, 1000):
                jobs = [
                    pool.submit(rec_fn, record.dict(), record.molecule) for _, _, record in tqdm(data_set_iodines.iterate_records(specification_names=["wb97x-d/def2-tzvpp"], status="complete", entry_names=batch), desc="Building Job list", total=len(batch)) 
                ]
                for result in tqdm(as_completed(jobs), desc="Building local dataset ..."):
                    rec_data = result.result()
                    rec_batch = pyarrow.RecordBatch.from_pylist([rec_data,], schema=schema)
...
if __name__ == "__main__":
    ...
    main(output="./gas_full_set.parquet")
