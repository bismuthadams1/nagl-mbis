...
def configure_data() -> DataConfig:
    ...
    return DataConfig(
        training=Dataset(
            sources=["/mnt/storage/nobackup/nca121/test_data_sets/gas/gas/training_gas_esp.parquet"],
        ...
        validation=Dataset(
            sources=["/mnt/storage/nobackup/nca121/test_data_sets/gas/gas/validation_gas_esp.parquet"],
            targets=[
                ReadoutTarget(
                    column="mbis-charges",
                    readout="mbis-charges",
        ...
        test=Dataset(
            sources=["/mnt/storage/nobackup/nca121/test_data_sets/gas/gas/testing_gas_esp.parquet"],
            targets=[
                ReadoutTarget(
                    column="mbis-charges",
                    readout="mbis-charges",
...
def main():
    ...
    dir_path = "/mnt/storage/nobackup/nca121/nagl-mbis-training/scripts/train_esp_model/full_iodine_set/default_0_5A/gas"
    output_dir = pathlib.Path(dir_path + "/001-train-charge-model-small-mols")
    ...
    logger = MLFlowLogger(
        ...
        tracking_uri=f"file://{(output_dir / 'mlruns').resolve()}",
        log_model="all",
    )
